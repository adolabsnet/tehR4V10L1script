...
function main_menu () {
    ...
    echo -e " ${white_color}[${normal_color}=================================${white_color}]${normal_color}"
    echo -e "${red_color_slim}  001)${white_color} Deauth"
    echo -e " ${white_color}[${normal_color}=================================${white_color}]${normal_color}"
...
function aireplay_deauth() {
    ...
    CHECKMON=$(ifconfig | grep "mon")
        if [[ "$CHECKMON" = "" ]]
        then
            echo -e "Managed mode enabled, I can't run the deauth!"
            echo -e "${normal_color}Do you want to activate monitor mode in ${white_color}"
            echo -ne "${white_color}  Press ${red_color_slim}[ENTER]${white_color} to continue.${normal_color}"
            read -p ""
            main_menu
        else
